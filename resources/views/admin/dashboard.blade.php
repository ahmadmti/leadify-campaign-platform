            ...
            <div class="card-wrap">
                ...
                <div class="card-body">
                    <p style="font-size: 12px; font-weight: 700;">
                        {{ $totalTimes > 0 ? \App\Classes\Common::secondsToStr($totalTimes) : '-' }}</p>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card gradient-bottom">
            ...
            <div class="card-body" id="top-5-scroll">
                <ul class="list-unstyled list-unstyled-border">
                    ...
                    <li class="media">
                        <img class="mr-3 rounded" width="55" src="{{ asset('assets/img/products/product-3-50.png') }}"
                            alt="product">
                        <div class="media-body">
                            <div class="float-right">
                                <div class="font-weight-600 text-muted text-small">
                                    {{ $activeCampaignsDataList['totalLeads'] }} @lang('app.calls')</div>
                            </div>
                            <div class="media-title"><a
                                    href="{{ route('admin.campaigns.show', md5($activeCampaignsDataList['id'])) }}">{{ $activeCampaignsDataList['name'] }}</a>
                            </div>
                            <div class="mt-1">
                                ...
                                <div class="budget-price">
                                    ...
                                    <div class="budget-price-label">
                                        {{ ($activeCampaignsDataList['totalLeads'] - $activeCampaignsDataList['interested']) }}
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div class="card-header-action">
                    <a href="{{ route('admin.appointments.index') }}" class="btn btn-danger">@lang('app.viewAll') <i
                    ...
                    <table class="table table-striped">
                        ...
                        <tr>
                            <td class="font-weight-600"><a
                                    href="{{ route('admin.campaigns.show', md5($bookedAppointment->campaign_id)) }}">{{ $bookedAppointment->campaign_name }}</a>
                            </td>
                            <td>{{ trim($bookedAppointment->first_name .' ' . $bookedAppointment->last_name) }}</td>
                            <td>{{ $bookedAppointment->appointment_time->timezone($user->timezone)->format($user->date_format .' ' . $user->time_format) }}
                            </td>
                            <td>
                                ...
                                $url =
                                url("/meeting").'?st='.$code['salt'].'&iv='.$code['iv'].'&clp='.$code['ciphertext'];
                            ...
                            <a href="{{ route('admin.callmanager.lead', [md5($bookedAppointment->lead_id)]) }}"
                                ...
                                data-original-title="@lang('module_call_enquiry.goAndResumeCall')"><i
                                    class="fas fa-play" aria-hidden="true"></i></a>

                            <a href="javascript:void(0);" onclick="viewLead('{{ md5($bookedAppointment->lead_id) }}')"
                                class="btn btn-icon btn-info" data-toggle="tooltip"
                                data-original-title="@lang('module_call_enquiry.viewLead')"><i class="fas fa-eye"
                        ...
                        <li class="media">
                            <div class="media-body">
                                <div class="mb-1 float-right">
                                    <div class="buttons">
                                        <a href="{{ route('admin.callmanager.lead', [md5($pendingCallback->lead_id)]) }}"
                                            class="btn btn-icon btn-sm btn-success" data-toggle="tooltip"
                                            data-original-title="@lang('module_call_enquiry.goAndResumeCall')"><i
                                                class="fas fa-play" aria-hidden="true"></i></a>

                                        <a href="javascript:void(0);"
                                            onclick="viewLead('{{ md5($pendingCallback->lead_id) }}')"
                                            class="btn btn-icon btn-sm btn-info" data-toggle="tooltip"
                                            data-original-title="@lang('module_call_enquiry.viewLead')"><i
                                                class="fas fa-eye" aria-hidden="true"></i></a>
                                    </div>
                                </div>
                                <h6 class="media-title"><a
                                        href="{{ route('admin.campaigns.show', md5($pendingCallback->campaign_id)) }}">{{ $pendingCallback->campaign_name }}</a>
                                </h6>
                                <div class="text-small text-muted">
                                    @php($newName = \App\Classes\Common::getLeadDataByColumn($pendingCallback->lead_id,
                                    $nameArray))
                                    @if($newName !== '')
                                    {{ $newName }}
                                    @else
                                    {{ \App\Classes\Common::getLeadDataByColumn($pendingCallback->lead_id, $firstNameArray) }}
                                    {{ \App\Classes\Common::getLeadDataByColumn($pendingCallback->lead_id, $lastNameArray) }}
                                    @endif
                                    <div class="bullet"></div> <span
                                        class="text-primary">{{ $pendingCallback->callback_time->timezone($user->timezone)->format($user->date_format .' ' . $user->time_format) }}</span>
                                </div>
                            </div>
                        </li>
                        @endforeach
...
function viewLead(id) {
    var url = '{{ route('
    admin.callmanager.view - lead ', ': id ') }}';
...
var myChart = new Chart(ctx, {
    ...
    data: {
        labels: {
            !!json_encode($dateArray) !!
        },
        datasets: [{
            label: "{{ trans('app.calls') }}",
            data: {
                !!json_encode($leadCountArray) !!
            },
            borderWidth: 2,
