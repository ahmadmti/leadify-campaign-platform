    ...
    <div class="form-group row">
        ...
        <div class="col-sm-9">
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu" style="padding: 5px;width:500px;" role="menu" aria-labelledby="menu1">
                    <div class="row">
                        <div class="col-md-8">

                            <input type="text" disabled id="date" name="date" class="model input form-control" value="">
...
<div class="modal-footer bg-whitesmoke" style="  display: flex;flex-direction: column;">
    <row>
        <a href="{{ route('admin.callmanager.lead', [md5($appointment->lead_id)]) }}" class="btn btn-icon btn-success"
            data-toggle="tooltip" data-original-title="@lang('module_call_enquiry.goAndResumeCall')">GO & Resume
            call</a>

        <a id="viewLeadButton" class="btn btn-icon icon-left btn-info" href="javascript:void(0);"
            onclick="viewLead('{{ md5($appointment->lead_id) }}')"><i class="fas fa-eye"></i>
            @lang('module_campaign.viewLead')</a>
    </row>

    <row style="margin-top: 5px;">
        <button id="saveFormButton" type="submit" class="btn btn-icon icon-left btn-success"
            onclick="editAppointment({{ $appointment->id }});return false"><i class="fas fa-check"></i>
            @if($appointment->id != '') @lang('app.update') @else @lang('app.save') @endif</button>
        ...
        <button style="margin-top:4px" class="btn default" data-dismiss="modal"
            aria-hidden="true">@lang('app.cancel')</button>
    </row>
</div>
    ...
    $.easyAjax({
        ...
        success: function(response) {
            ...
            response.forEach(element => {
                ...
                if (element != null) {
                    $('#sales_member_id').append("<option value=" + element.id + ">" + element
                        .first_name + "</option>");
