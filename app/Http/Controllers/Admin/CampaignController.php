...
class CampaignController extends AdminBaseController
{
    ...
    public function show($id)
    {
        ...
        // return $this->campaignDetails->staffMembers->sortBy('total_leads', SORT_REGULAR, true)->first();;
        ...
        foreach ($this->campaignDetails->staffMembers as $campaignStaffMember)
        {
            ...
            $notInterestedLeadByUsers = Lead::where('campaign_id', $this->campaignDetails->id)
                ...
                $soldLeadByUsers = Lead::where('campaign_id', $this->campaignDetails->id)
                ->where('last_actioned_by', $campaignStaffMember->user_id)
                ->where('interested', 'sold')
                ->count(); 

                  $installedLeadByUsers = Lead::where('campaign_id', $this->campaignDetails->id)
                ->where('last_actioned_by', $campaignStaffMember->user_id)
                ->where('interested', 'installed')
                ->count(); 

                   $waitingforinstallationLeadByUsers = Lead::where('campaign_id', $this->campaignDetails->id)
                ->where('last_actioned_by', $campaignStaffMember->user_id)
                ->where('interested', 'waiting for installation')
                ->count(); 
            ...
            $campaignStaffMember->not_interested_leads = $notInterestedLeadByUsers;
             $campaignStaffMember->unreachable_leads = $unreachableLeadByUsers;
              ...
              $campaignStaffMember->sold_leads = $soldLeadByUsers;
             $campaignStaffMember->installed_leads = $installedLeadByUsers;
             $campaignStaffMember->waiting_for_installation_leads = $waitingforinstallationLeadByUsers;
